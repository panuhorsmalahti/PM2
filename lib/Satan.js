...
Satan._actions = [];
Satan._worker_timer  =  null;

var running = false;

Satan.syncWorker = function() {
  if (Satan._worker_timer !== null) return false;

  Satan._worker_timer = setInterval(function() {

    if (Satan._actions.length > 0 && running === false) {
      running = true;

      var method = Satan._actions[0].method;
      var opts   = Satan._actions[0].opts;
      var fn     = Satan._actions[0].fn;

      Satan.start(function() {
        Satan.client.call(method, opts, function() {
          var tmp = arguments;

          Satan.disconnectRPC(function() {
            fn && fn.apply(Satan, tmp);
            Satan._actions.shift();
            running = false;
            if (Satan._actions.length === 0)
              Satan.stopSyncWorker();
          });
...
Satan.stopSyncWorker = function() {
  clearInterval(Satan._worker_timer);
  Satan._worker_timer = null;
};

Satan._call = function(method, opts, fn) {
  debug('Calling daemon method pm2:%s', method);

  if (process.env.PM2_PROGRAMMATIC === 'true') {
    /**
     * If programmatic calls, make them synchronous
     */
    Satan._actions.push({
      method : method,
      opts : opts,
    ...
    return Satan.client.call(method, opts, fn);
  }
};
...
Satan.executeRemote = function executeRemote(method, env, fn) {
  ...
  return Satan._call(method, env, fn);
};
